  ...
  (if-let [account (find-account-by-username db username)]
    (if (crypto/check password (:account/password account))
      (do
        ...
        (clients/produce (get-in db [:kafka-p :producer]) cac-topic (ConfirmAccountCreation. (Uuid. (uuid->bytes (:account/uuid account))) Atype/MANUAL)))
    ...
    (let [uuid (UUID/randomUUID)
      ...
      (clients/produce (get-in db [:kafka-p :producer]) cac-topic (ConfirmAccountCreation. (Uuid. (uuid->bytes uuid)) Atype/MANUAL))
      sub-id)))
